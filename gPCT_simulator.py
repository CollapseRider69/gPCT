...
The CHSH panel shows two curves: the underlying slope-conditioned
correlation (emitter frame) and its polarity-inverted observer-side
projection.
...
# Diagnostics
print("=" * 72)
print("gPCT v6 — Phase-address worldlines")
...
fig.suptitle(
    "gPCT v6 — Phase-Address Worldlines\n"
...
# Two curves: the underlying slope-conditioned correlation (emitter frame)
# and its polarity-inverted observer-side projection (external frame).
#
#   S_emitter(p)  = 1 + √2 + (√2 − 1) · sin(2π|p|)   [underlying, direct from P(|1⟩)]
#   S_observer(p) = 1 + √2 − (√2 − 1) · sin(2π|p|)   [observer-side projection]
#
# Both oscillate between 2 and 2√2. Average over full cycle = 1 + √2 ≈ 2.4142.
# Each crosses 1 + √2 exactly four times per cycle.
...
plt.savefig("gPCT_v6_SIMULATOR.png", dpi=150, bbox_inches="tight")
plt.show()
print("\nSaved: gPCT_v6_SIMULATOR.png")
